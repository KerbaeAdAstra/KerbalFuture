...
namespace KerbalFuture.Superluminal.SpaceFolder
{
	public class SpaceFolderDriveVesselModule : VesselModule
    {
		...
		public bool WarpVessel(SpaceFolderWarpData warpData, out Error fault)
        {
            ...
            UseWarpResources();
            Vessel.SetPosition(new Coords(warpData.WarpLocation.Lat, warpData.WarpLocation.Lon, WarpHelp.CalculateGravPot(warpData.WarpLocation.Body, Vessel), warpData.WarpLocation.Body).WorldSpace);
            DistributeHeat();
            return true;
        }
